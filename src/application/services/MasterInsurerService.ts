...
export class MasterInsurerService {
  ...
  async updateMasterInsurer(id: string, dto: MasterInsurerDTO): Promise<BaseResponse<MasterInsurerDTO | null>> {
    try {
      await MongoDBConnectionService()
      const existingInsurer = await this.masterInsurerRepository.findById(id)
      if (!existingInsurer) {
        return {
          statusCode: 404,
          message: 'MasterInsurer not found',
          data: null,
        }
      }

      const updatedDomain = this.mapToDomain({ ...dto, id })
      const updatedInsurer = await this.masterInsurerRepository.update(id, updatedDomain)
      return {
        statusCode: updatedInsurer ? 200 : 500,
        message: updatedInsurer ? 'MasterInsurer updated successfully' : 'Failed to update MasterInsurer',
        data: updatedInsurer ? this.mapToDTO(updatedInsurer) : null,
      }
    } catch (error) {
      console.error(`Error updateMasterInsurer :`, error)
      return {
        statusCode: 500,
        message: 'Failed to update MasterInsurer',
        data: null,
      }
    }
  }
